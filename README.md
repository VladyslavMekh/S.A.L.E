...
* **[Sort]:** The function sorts elements based on the passed attributes. Outside attributes contained in the element, the function should allow you to sort the set using: average grade, minimum grade, maximum grade or name and surname length. In addition to information about the sort type (asc or desc) can be added to each attribute. Order specifying attributes should influence the priority of a given attribute in sorting. If not attributes passed, the operation should be skipped.

* **[Export to JSON]:** Finally, formats the sorted list and saves it to a file raport.json.
